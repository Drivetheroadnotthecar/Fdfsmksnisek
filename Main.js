...
// Middleware to detect API calls (for JSON error handling)
app.use((req, res, next) => {
  req.isApi = ['/register', '/login-user'].includes(req.path);
  next();
});
...
app.post('/ban-user', loginRequired, (req, res) => {
  ...
  db.get("SELECT hwid, ip FROM users WHERE username=?", [username], (err, row) => {
    ...
    db.run("UPDATE users SET status='banned' WHERE username=?", [username], err2 => {
...
app.post('/unban-user', loginRequired, (req, res) => {
  ...
  db.get("SELECT hwid, ip FROM users WHERE username=?", [username], (err, row) => {
    ...
    db.run("UPDATE users SET status='active' WHERE username=?", [username], err2 => {
      if (err2) return res.status(500).send("DB error");
      res.redirect('/');
    });
  });
...
app.get('/all-users', loginRequired, (req, res) => {
  db.all("SELECT username, password, hwid, ip, status, note FROM users", (err, rows) => {
    if (err) return res.status(500).send("DB error");
    let html = `<h2>Registered Users</h2><table border="1">
      <tr><th>Username</th><th>Password Hash</th><th>HWID</th><th>IP</th><th>Status</th><th>Note</th></tr>`;
    rows.forEach(r => {
      html += `<tr>
...
app.post('/login-user', (req, res) => {
  ...
  db.get("SELECT password, hwid, ip, status FROM users WHERE username=?", [username], async (err, row) => {
    ...
    try {
      ...
      if (!match) return res.status(403).json({ status: "error", message: "Invalid credentials." });
...
app.use((req, res, next) => {
  if (req.isApi) return res.status(404).json({ status: 'error', message: 'API endpoint not found' });
  next();
});
